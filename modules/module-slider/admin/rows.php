 ...
 * @Createdate Fri, 01 Mar 2019 17:17:16 GMT
 */
...
$row = array();
$error = array();
$row['sliderid'] = $nv_Request->get_int('sliderid', 'post,get', 0);
$row['id'] = $nv_Request->get_int('id', 'post,get', 0);
if ($nv_Request->isset_request('submit', 'post')) {
    $row['sid'] = $nv_Request->get_int('sliderid', 'post,get', 0);
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $stmt = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_rows (sid, title, alias, content, status, weight) VALUES (:sid, :title, :alias, :content, :status, :weight)');
                ...
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_rows SET sid = :sid, title = :title, alias = :alias, content = :content WHERE id=' . $row['id']);
            }
            $stmt->bindValue(':sid', $row['sid'], PDO::PARAM_INT);
...
} elseif ($row['sliderid'] > 0) {
    $slider_info = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . ' WHERE id=' . $row['sliderid'])->fetch();
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    if (!empty($q)) {
        $db->where('title LIKE :q_title OR content LIKE :q_content');
    }
    $sth = $db->prepare($db->sql());

    if (!empty($q)) {
        $sth->bindValue(':q_title', '%' . $q . '%');
        $sth->bindValue(':q_content', '%' . $q . '%');
    }
    $sth->execute();
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['link_sub'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=rows-items&amp;rowid=' . $view['id'];
